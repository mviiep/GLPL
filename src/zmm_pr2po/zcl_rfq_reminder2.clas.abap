CLASS zcl_rfq_reminder2 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_RFQ_REMINDER2 IMPLEMENTATION.
...
** Reminder 2
    ...
    LEFT OUTER JOIN zdb_rfqdate                 AS rfqdate ON rfqdate~requestforquotation EQ supmailtab~requestforquotation
    WHERE reminder_flg1 EQ 'X'
    AND reminder_flg2 NE 'X'
...
**Subject of Email
          lo_mail->set_subject( |Reminder 2 - RFQ: { wa_rfqsuppliers_r1-requestforquotation } [{ lv_purchaserequisition } - {  wa_rfqsuppliers_r1-requestforquotationname }]. | ).
...
**Body of Email
          ...
          | The details of the RFQ were shared on <strong>{ date }</strong>, and we had requested your submission by <strong>{ originaldate }</strong>.</p> <p>If there are any issues or clarifications required regarding the RFQ, | &&
          | please do not hesitate to reach out to us. We would be happy to assist to ensure the timely submission of your quotation.</p> | &&
          |<p>We kindly request you to share your quotation by <strong>{ newdate }</strong>.</p> | &&
          |<p>Thank you for your prompt attention to this matter, and we look forward to receiving your response.</p> | &&
          |<p>Sincerely,</br> Godrej Living Pvt Ltd</strong></p> |.
...
**Send Mail
      ...
      IF wa_rfqsuppliers_r1_response-status EQ 'S'.
        ...
        wa_rfqsuppliers_r1-reminder_flg2 = 'X'.

**Update the Data with Reminder 2 Field as X
        UPDATE zirfq_suppmail SET reminder_flg2 = @wa_rfqsuppliers_r1-reminder_flg2
        WHERE requestforquotation = @wa_rfqsuppliers_r1-requestforquotation
