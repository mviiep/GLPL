CLASS zcl_pr_update_schedule DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_PR_UPDATE_SCHEDULE IMPLEMENTATION.
  ...
  METHOD update_pr.
    ...
    LOOP AT it_pr INTO DATA(ls_pr).

      lv_test     =   ls_pr-lastchangedatetime.
      DATA(datetest) = lv_test+0(8).
      IF datetest NE lv_current_date.
        DELETE it_pr WHERE purchaserequisition = ls_pr-purchaserequisition.
      ENDIF.
    ENDLOOP.

    DELETE ADJACENT DUPLICATES FROM it_pr COMPARING purchaserequisition purchaserequisitionitem.
    SELECT DISTINCT purchaserequisition
        FROM i_purchaserequisitionapi01
        FOR ALL ENTRIES IN @it_pr
        WHERE purchaserequisition = @it_pr-purchaserequisition
        INTO TABLE @DATA(it_prmain).
    SORT it_pr BY purchaserequisition purchaserequisitionitem ASCENDING.
    LOOP AT it_prmain INTO DATA(wa_prmain).
      LOOP AT it_pr INTO DATA(wa_pr) WHERE purchaserequisition = wa_prmain-purchaserequisition.
        ...
        LOOP AT it_po INTO DATA(wa_po) WHERE purchaserequisition = wa_pr-purchaserequisition AND purchaserequisitionitem = wa_pr-purchaserequisitionitem.
          SELECT SINGLE * FROM i_purchaseorderapi01 WHERE purchaseorder = @wa_po-purchaseorder INTO @DATA(wa_pohead).
      ...
      CLEAR: wa_data,lv_json_data-data.
...
**********************************************************************
      ...
      IF sy-subrc EQ 0.
        ...
        wa_table-purchaserequisition  = wa_pr-purchaserequisition.
        wa_table-prlastchangedatetime = wa_pr-lastchangedatetime.

        SELECT SINGLE *
          FROM zpr_zohostatus
          WHERE purchaserequisition = @wa_pr-purchaserequisition
          INTO @DATA(wa_prstatus).
        IF wa_prstatus-purchaserequisition IS INITIAL.
          MODIFY zpr_zohostatus FROM @wa_table.
        ELSE.
          UPDATE zpr_zohostatus SET message = 'Data Updated Successfully' WHERE purchaserequisition = @wa_pr-purchaserequisition.
        ENDIF.
      ELSE.


        wa_table-flag = ''.
        wa_table-message = 'Data not Updated '.
        wa_table-purchaserequisition = wa_pr-purchaserequisition.

        SELECT SINGLE * FROM zpr_zohostatus WHERE purchaserequisition = @wa_pr-purchaserequisition INTO @wa_prstatus.
        IF wa_prstatus-purchaserequisition IS INITIAL.
          MODIFY zpr_zohostatus FROM @wa_table.
        ELSE.
          UPDATE zpr_zohostatus SET message = 'Data Updated Successfully', prlastchangedatetime = @wa_pr-lastchangedatetime WHERE purchaserequisition = @wa_pr-purchaserequisition.
        ENDIF.
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
