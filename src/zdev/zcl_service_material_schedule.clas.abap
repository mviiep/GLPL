CLASS zcl_service_material_schedule DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_SERVICE_MATERIAL_SCHEDULE IMPLEMENTATION.
...
*       WHERE creationdate = @sy-datum
    ...
    SORT it_data BY product.
    SORT it_mat BY matnr.

    LOOP AT it_data INTO DATA(gs_data).
      READ TABLE it_mat INTO DATA(gs_mat) WITH KEY matnr = gs_data-product BINARY SEARCH.
      IF sy-subrc = 0.
        DELETE it_data WHERE product = gs_mat-matnr.
      ENDIF.
      CLEAR : gs_mat, gs_data.
    ENDLOOP.
    ...
    LOOP AT it_data INTO DATA(wa_data).
      ...
      IF wa_check-zoho_material IS INITIAL.
        ...
        ENDDO.
*          IF wa_data-res_matnr IS INITIAL.
...
*          ENDIF.
        CLEAR : wa_data,  lv_payload, lo_request, lv_token, status, lv_response, lv_response_text, lr_data,
                wa_mat, lv_msg, lv_msg1, lv_msg2, lv_text,  prdct_status , ls_material, lt_purchasetax, lt_salestax,
                wa_jsondata-data, lt_material, lv_json, lv_count, lt_plant, lt_sales, lt_valuation, lt_descr. " wa_record, wa_msg ,
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
