CLASS lhc_zi_customer_update DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
**********************************************************************
    ...
    METHODS get_instance_features FOR INSTANCE FEATURES
      IMPORTING keys REQUEST requested_features FOR zi_customer_update RESULT result.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR zi_customer_update RESULT result.

    METHODS create FOR MODIFY
      IMPORTING entities FOR CREATE zi_customer_update.

    METHODS update FOR MODIFY
      IMPORTING entities FOR UPDATE zi_customer_update.

    METHODS delete FOR MODIFY
      IMPORTING keys FOR DELETE zi_customer_update.

    METHODS read FOR READ
      IMPORTING keys FOR READ zi_customer_update RESULT result.

    METHODS lock FOR LOCK
      IMPORTING keys FOR LOCK zi_customer_update.

    METHODS update_customer FOR MODIFY
      IMPORTING keys FOR ACTION zi_customer_update~update_customer RESULT result.

ENDCLASS.

CLASS lhc_zi_customer_update IMPLEMENTATION.
...
*    READ ENTITIES OF ZI_CUSTOMER_UPDATE IN LOCAL MODE
*    ENTITY ZI_CUSTOMER_UPDATE
...
*    REPORTED DATA(reported_data).
    ...
    SELECT SINGLE *
        ...
        WHERE customer = @wa_key-customer
        INTO  @DATA(wa_zoho).

    SELECT  customerpaymentterms, division
        FROM i_customersalesarea
        WHERE customer = @wa_key-customer
        INTO TABLE @DATA(it_zoho).
    SELECT SINGLE * FROM i_salesdistricttext
*         FOR ALL ENTRIES IN @it_zoho
         WHERE salesdistrict = @wa_zoho-salesdistrict
           AND language = 'E'
         INTO  @DATA(wa_salesdistrict).
    IF it_zoho IS NOT INITIAL.
      ...
      SELECT paymentterms, paymenttermsname FROM i_paymenttermstext
          FOR ALL ENTRIES IN @it_zoho
          WHERE paymentterms = @it_zoho-customerpaymentterms
            AND language = 'E'
          INTO TABLE @DATA(it_payment).
    ENDIF.
*    select single customer, deletionindicator
*        FROM i_businesspartnercustomerdex
*        where Customer = @wa_zoho-Customer
*        into @data(wa_status).
    IF wa_zoho IS NOT INITIAL.
      ...
      ENDCASE.
      LOOP AT it_zoho INTO DATA(wa_zoho1).
...
*    lo_url = 'https://glpl-nonprod-apim-02.azure-api.net/Accounts/Accounts/723303000001085658'.
    ...
    IF sy-subrc EQ 0.
      ...
      ELSE.
        UPDATE zcust_zoho_statu
          ...
          WHERE customer = @wa_zoho-customer.
      ENDIF.
      DATA wa_record LIKE LINE OF reported-zi_customer_update.
      wa_record-%msg = wa_msg.
      wa_record-%tky-%key-customer = wa_zoho-customer.
      APPEND wa_record TO reported-zi_customer_update.
...
CLASS lsc_zi_customer_update DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_zi_customer_update IMPLEMENTATION.
