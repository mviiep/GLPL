CLASS zcl_credit_form DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_final,
             ...
             lv_data(2)                     TYPE c,
*             DOCUMENTREFERENCEID1            type   i_accountingdocumentjournal-DocumentReferenceID,
             documentdate5                  TYPE  i_accountingdocumentjournal-documentdate,
...
CLASS zcl_credit_form IMPLEMENTATION.
  ...
  METHOD get_obj.
    ...
    CASE sy-sysid.
      WHEN lv_dev.
        lo_request->set_uri_path( i_uri_path = '/v1/forms/Credit_dev' ).
      WHEN lv_qas.
        lo_request->set_uri_path( i_uri_path = '/v1/forms/credit_qas' ).
      WHEN lv_prd.
        lo_request->set_uri_path( i_uri_path = '/v1/forms/credit_qas' ).
    ENDCASE.
...
*****************************POST METHOD*****************************
    ...
    IF objectid IS NOT INITIAL AND xmldata IS NOT INITIAL AND r_token IS NOT INITIAL.
      ...
      CASE sy-sysid.
        WHEN lv_dev.
          DATA(ls_body) = VALUE struct(  xdp_template = 'Credit_dev/' && |{ objectid }|
                                               xml_data  = xmldata ).
        WHEN lv_qas.
          ls_body = VALUE struct(  xdp_template = 'credit_qas/' && |{ objectid }|
                                               xml_data  = xmldata ).
        WHEN lv_prd.
          ls_body = VALUE struct(  xdp_template = 'credit_qas/' && |{ objectid }|
                                               xml_data  = xmldata ).
...
*    INTO TABLE @DATA(lt_cust).
    ...
    WHERE accountingdocument = @lv_account
          AND fiscalyear = @lv_supp
*     AND  invoicereferencefiscalyear = @lv_supp
          AND financialaccounttype = 'D'
          INTO TABLE @DATA(lt_ghdocument).

    IF lt_ghdocument IS NOT INITIAL.
      ...
      SELECT accountingdocument,
             ...
             fiscalyear
             FROM i_accountingdocumentjournal
             FOR ALL ENTRIES IN  @lt_ghdocument
             WHERE accountingdocument = @lt_ghdocument-invoicereference
             and FiscalYear = @lt_ghdocument-InvoiceReferenceFiscalYear
*             AND fiscalyear = @lv_supp
             INTO TABLE @DATA(lt_jou).
...
*              INTO TABLE @DATA(lt_sales).
  ...
  INTO TABLE @DATA(lt_sales).
      ...
      SELECT FROM i_journalentryitem FIELDS accountingdocument ,  financialaccounttype, transactiontypedetermination ,debitamountincocodecrcy,referencedocumentitem, fiscalyear   "unitrate
      FOR ALL ENTRIES IN @lt_cust
      WHERE accountingdocument =  @lt_cust-accountingdocument
      AND referencedocumentitem = @lt_cust-referencedocumentitem
      AND financialaccounttype = 'S'
      ...
      SELECT FROM i_journalentryitem FIELDS accountingdocument ,  financialaccounttype, transactiontypedetermination , debitamountincocodecrcy,referencedocumentitem, fiscalyear  "cgstrate
      FOR ALL ENTRIES IN @lt_cust
      WHERE accountingdocument =  @lt_cust-accountingdocument
*       and ReferenceDocumentItem = @lt_cust-ReferenceDocumentItem
      AND financialaccounttype = 'S'
      ...
      SELECT FROM i_journalentryitem FIELDS accountingdocument ,  financialaccounttype, transactiontypedetermination , debitamountincocodecrcy,referencedocumentitem,fiscalyear
   FOR ALL ENTRIES IN @lt_cust
   WHERE accountingdocument =  @lt_cust-accountingdocument
*    and ReferenceDocumentItem = @lt_cust-ReferenceDocumentItem
   AND financialaccounttype = 'S'
   ...
   INTO TABLE @DATA(it_sgstrate).
      ...
      SELECT FROM i_journalentryitem FIELDS accountingdocument ,  financialaccounttype, transactiontypedetermination , debitamountincocodecrcy,referencedocumentitem, fiscalyear    "igstrate
 FOR ALL ENTRIES IN @lt_cust
 WHERE accountingdocument =  @lt_cust-accountingdocument
*  and ReferenceDocumentItem = @lt_cust-ReferenceDocumentItem
 AND financialaccounttype = 'S'
 ...
 INTO TABLE @DATA(it_igstrate).
      ...
      SELECT FROM i_journalentryitem FIELDS accountingdocument ,  financialaccounttype, transactiontypedetermination , debitamountincocodecrcy,referencedocumentitem,fiscalyear     "Ugstrate
 FOR ALL ENTRIES IN @lt_cust
 WHERE accountingdocument =  @lt_cust-accountingdocument
*  and ReferenceDocumentItem = @lt_cust-ReferenceDocumentItem
 AND financialaccounttype = 'S'
...
********************************************
    ...
    LOOP AT lt_cust INTO DATA(wa_cust).
      ...
      wa_final-creationdate = wa_cust-documentdate.
      wa_final-bankacc  = wa_cust-documentreferenceid.
      ...
      READ TABLE lt_ghdocument INTO DATA(wa_ghdocument) WITH KEY accountingdocument = wa_cust-accountingdocument.
      IF sy-subrc = 0.

      ENDIF.

      READ TABLE lt_jou INTO DATA(wa_jou) WITH KEY accountingdocument = wa_ghdocument-invoicereference.

      IF sy-subrc = 0.
        wa_final-documentreferenceid  = wa_jou-documentreferenceid.
      ENDIF.
...
*        wa_final-lv_date2      = wa_sales-yy1_fromdate_sdh.
      ...
      IF sy-subrc = 0.
        wa_final-creditamountincocodecrcy  = wa_unitrate-debitamountincocodecrcy * 1.
...
*      ENDIF.
        ...
        IF wa_cgstrate-transactiontypedetermination = 'JOC'.
          ...
          wa_final-creditamountincocodecrcy1  = wa_cgstrate-debitamountincocodecrcy * 1.
      ...
      LOOP AT it_cgstrate INTO wa_cgstrate  WHERE accountingdocument = wa_cust-accountingdocument .
        wa_final-creditamountincocodecrcy5 = wa_final-creditamountincocodecrcy5 + wa_cgstrate-debitamountincocodecrcy.
...
*                                                          ReferenceDocumentItem = wa_cust-ReferenceDocumentItem
      ...
      IF sy-subrc = 0.
        ...
        IF wa_sgstrate-transactiontypedetermination = 'JOS'.
          ...
          wa_final-creditamountincocodecrcy2  = wa_sgstrate-debitamountincocodecrcy * 1.
...
*         CONDENSE wa_final-creditamountincocodecrcy2.
      ...
      LOOP AT it_sgstrate INTO wa_sgstrate  WHERE accountingdocument = wa_cust-accountingdocument .
        wa_final-creditamountincocodecrcy6 = wa_final-creditamountincocodecrcy6 +  wa_sgstrate-debitamountincocodecrcy.
...
*                                                           ReferenceDocumentItem = wa_cust-ReferenceDocumentItem
      ...
      IF  sy-subrc = 0.
        ...
        IF wa_igstrate-transactiontypedetermination = 'JOI'.
          ...
          wa_final-creditamountincocodecrcy3  = wa_igstrate-debitamountincocodecrcy * 1.
      ...
      LOOP AT it_igstrate INTO wa_igstrate WHERE accountingdocument = wa_cust-accountingdocument .
        wa_final-creditamountincocodecrcy7 = wa_final-creditamountincocodecrcy7 +  wa_igstrate-debitamountincocodecrcy.
...
*                                                           ReferenceDocumentItem = wa_cust-ReferenceDocumentItem
      ...
      IF sy-subrc = 0.
        ...
        IF wa_ugstrate-transactiontypedetermination = 'JOU'.
          ...
          wa_final-creditamountincocodecrcy2  = wa_ugstrate-debitamountincocodecrcy * 1.
          wa_final-sgstrate   = wa_final-creditamountincocodecrcy2 * 100 / wa_final-creditamountincocodecrcy.
      ...
      LOOP AT it_ugstrate  INTO wa_ugstrate WHERE accountingdocument = wa_cust-accountingdocument .
        wa_final-creditamountincocodecrcy6 = wa_final-creditamountincocodecrcy6 + wa_ugstrate-debitamountincocodecrcy.
...
*   LV_RES = WA_FINAL-total.
      ...
      APPEND wa_final TO it_final.  " ( Credit note form )
      ...
      wa_operation,wa_placesupply,wa_bankdetails, lv_round,wa_ghdocument, wa_jou.
*      text1 , text2, lv_a9.
    ENDLOOP.
